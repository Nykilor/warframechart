    ...
    public function save() : void {
        ...
        foreach ($this->data as $type => $value) {
            ...
            foreach ($value as $item => $entity) {
                $table .= $this->createTableRow($entity);
            }
            $table .= $this->getTableFooter();

            $doc .= $this->createDocBody($table, $type);

            $this->createFileIfNotExists($this->path, $type.".html");
            $this->saveToFile($this->path.$type.".html", $doc);

        }
    }

    private function createTableRow(\Entity\Statistics $entity) : string {
        $name = $entity->getName();
        $min = $entity->getMin();
        $max = $entity->getMax();
        $avg = $entity->getAvg();
        $median = $entity->getMedian();
        $mode = $entity->getMode();
...
THEDOC;
    ...
    private function createDocBody(string $body_content, $type) : string {
        if($type === "sell") {
            $cross = "buy";
        } else {
            $cross = "sell";
        }
        return "<body><a href='".$cross.".html'>GO TO ".strtoupper($cross)."</a><h1>".$type." statistics</h1>".$body_content."</body></html>";
    }
}
