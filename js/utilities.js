 ...
 * @return {Boolean}
 */
export function isMultidimensional(array) {
...
export function createRange(start, edge, step) {
  ...
  let ret = [];
  for (let i = 0; (edge - start) * step > 0; start += step) {
    ret.push(start);
  }
  return ret;
};
 ...
 */
export function createYMDDate(days, fromDate = new Date()) {
  fromDate.setDate(fromDate.getDate() + days);
  return fromDate.toISOString().substr(0, 10);
};

export function deleteZeros(variable) {
  let values = [];
...
export function findMode(numbers) {
